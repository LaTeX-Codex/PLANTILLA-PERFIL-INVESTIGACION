...
\newcommand{\tipoinvestigacion}{Proyecto Informático} % Tipo de investigación
\newcommand{\tituloopta}{Ingeniero(a) en Sistemas} % Título al que se opta
\newcommand*{\thead}[1]{\multicolumn{1}{>{\centering\arraybackslash}X}{\bfseries #1}} % Encabezado centrado y en negritas en tablas

\usepackage{lipsum} % Paquete para generar texto de ejemplo (puedes eliminarlo si no lo necesitas)

\begin{document}

\input{sections/00_portada.tex}
\input{sections/01_tema.tex}
\input{sections/02_diagnostico_situacional.tex}
\input{sections/03_alternativa.tex}
